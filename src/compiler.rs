...
pub fn test() {
    ...
    let mut ir_container: Box<jitter::JITter> = Box::new(jitter::MCJITter::new("main"));

    let mut context = Context::new();

    let test_expression = "(+ 1 2)";
    let node = parser::tokenize(&test_expression).unwrap();
    //let node = atom::Atom::Number(Number::Integer(0));
    match node.codegen(&mut context, ir_container.get_module_provider()) {
        Ok((value, runnable)) => {
            if runnable {
                let r = ir_container.run_function(value);
                println!("r = {:?}", r);
           } else {
                ir_container.dump();
            }
...
impl IRBuilder for Vec<atom::Atom> {
    fn codegen(&self, context: &mut Context, module_provider: &mut ModuleProvider) -> IRBuildingResult {
...
impl IRBuilder for atom::Atom {
    fn codegen(&self, context: &mut Context, module_provider: &mut ModuleProvider) -> IRBuildingResult {
        ...
        println!("codegen for: {}", self);
        match *self {
            atom::Atom::Number(Number::Integer(d)) => {
                return Ok((RealConstRef::get(&context.ty, d as f64).to_ref(), false))
            },
            atom::Atom::List(ref l) => {
                match l[0] {
                    atom::Atom::Function(atom::Function::Native(_)) => {
                        let (x, _) = try!(l[1].codegen(context, module_provider));
                        let (y, _) = try!(l[2].codegen(context, module_provider));
                        return Ok((context.builder.build_fadd(x, y, "addtmp"), false))
                    },
                    _ => ()
                }
            }

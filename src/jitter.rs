...
//< jit-jitter
pub trait JITter : compiler::ModuleProvider {
    // TODO: fix https://github.com/rust-lang/rust/issues/5665
    fn get_module_provider(&mut self) -> &mut compiler::ModuleProvider;
...
//< jit-mcjitter-ctor
    pub fn new(name: &str) -> MCJITter {
        let (current_module, function_passmanager) = compiler::new_module(name);
...
//< jit-mcjitter-new-module
        let (new_module, new_function_passmanager) = compiler::new_module(&self.module_name);
...
impl compiler::ModuleProvider for MCJITter {
...
//< jit-smp
impl JITter for compiler::SimpleModuleProvider {
    fn get_module_provider(&mut self) -> &mut compiler::ModuleProvider {
        self
    }
